...
class RepositoryDB extends BaseRepository implements RepositoryInterface
{
  ...
  public function exists($id): bool
  {
    ...
    $stmt = $this->pdo->prepare($sql);
    $stmt->execute(['id' => $id]); //! Así se prepara la consulta para que no se haga una inyección SQL.
  ...
  {
    $sql = "UPDATE " . self::TABLE . " SET name = :name, alcohol = :alcohol, idBrand = :idBrand " . "WHERE id = :id";
    $stmt = $this->pdo->prepare($sql);
    $stmt->execute($data);
  }
